...
class SR {
    ...
    private long base = 0;
    private final int virtualLossRemainder = 17; // 用于模拟数据报丢失的余数
    ...
    /** 
    * @Description: transport content data to host:targetPort 
    * @Param:  
    * @return:  
    * @Author: 1170300421 
    * @Date: 2019/11/1 
    */ 
    void send(byte[] content) throws IOException {
        ...
        DatagramSocket datagramSocket = new DatagramSocket(ownPort);//套接字
        ...
        do {
            while (timers.size() < WindowSize && sendIndex < content.length && sendSeq < 256) { // until the window is run up
                ...
                ByteArrayOutputStream oneSend = new ByteArrayOutputStream();//发送方数据缓冲区
                byte[] temp = new byte[1];
                temp[0] = new Long(base).byteValue();
                oneSend.write(temp, 0, 1);//字节写入缓冲区
                temp = new byte[1];
    ...
    /** 
    * @Description: receive data from host:targetPort 
    * @Param:  
    * @return:  
    * @Author: 1170300421 
    * @Date: 2019/11/1 
    */ 
    ...
    /** 
    * @Description: splice the ByteArrays(datagram) to one ByteArray(one datagram) 
    * @Param:  
    * @return:  
    * @Author: 1170300421 
    * @Date: 2019/11/1 
   ...
   /** 
   * @Description: check if it's ok to slide window
   * @Param:  
   * @return:  
   * @Author: 1170300421 
   * @Date: 2019/11/1 
